...
public class Rsa {
    ...
    public static boolean doCheck256(String content, String sign, String publicKey) {
        try {
            return doCheck256(content.getBytes(CHARSET), sign, publicKey);
        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
        }
        return false;
     ...
     * @return base64编码后的签名字符串
     */
    public static String signByPrivateKey256(byte[] content, String privateKey) {
        try {
            PKCS8EncodedKeySpec priPKCS8 = new PKCS8EncodedKeySpec(Base64.decode(privateKey, Base64.NO_WRAP));
            KeyFactory keyf = KeyFactory.getInstance(ALGORITHM);
            PrivateKey priKey = keyf.generatePrivate(priPKCS8);
     ...
     * @return true表示签名一致
     */
    public static boolean doCheck256(byte[] content, String sign, String publicKey) {
        try {
            KeyFactory keyFactory = KeyFactory.getInstance(ALGORITHM);
            byte[] encodedKey = Base64.decode(publicKey, Base64.NO_WRAP);
            PublicKey pubKey = keyFactory.generatePublic(new X509EncodedKeySpec(encodedKey));
     ...
     * @return 解密后内容
     */
    public static String decryptByPublicKeyByte(String data, String key) {
        try {
